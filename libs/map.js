class Map {
	...
	delBucket(...args) {
		...
		if (aLen == 1) {
			...
			const arr = Array.from(args);
			const parentMapping = arr.slice(0, arr.length - 1);
			const keyToDelete = arr[arr.length - 1];
			const parentBucket = this.getBucket(...parentMapping);
			if (parentBucket && parentBucket.hasOwnProperty(keyToDelete)) {
				delete parentBucket[keyToDelete];
				if (Object.keys(parentBucket).length === 0) {
					this.delBucket(...parentMapping);
				}
			}
		} else {
			throw new Error("invalid key");
