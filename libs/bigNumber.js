...
! function(e) {
	...
	function d(e, r, n, t) {
		if (e < r || e > n || e !== i(e)) throw Error(o + (t || "Argument") + ("number" == typeof e ? e < r || e > n ?
	...
	r = function e(r) {
		...
		function z(e, r) {
			...
			if (null == r) {
				if (e && !0 === e._isBigNumber) return w.s = e.s, void(!e.c || e.e > M ? w.c = w.e = null : e.e <
					C ? w.c = [w.e = 0] : (w.e = e.e, w.c = e.c.slice()));
				...
				}(c = p.indexOf(".")) > -1 && (p = p.replace(".", "")), (a = p.search(/e/i)) > 0 ? (c < 0 && (c =
					a), c += +p.slice(a + 1), p = p.substring(0, a)) : c < 0 && (c = p.length)
			} else {
				if (d(r, 2, j.length, "Base"), 10 == r && $) return X(w = new z(e), L + w.e + 1, U);
				if (p = String(e), h = "number" == typeof e) {
					if (0 * e != 0) return b(w, p, h, r);
					if (w.s = 1 / e < 0 ? (p = p.slice(1), -1) : 1, z.DEBUG && p.replace(/^0\.0*|\./, "").length >
						15) throw Error(s + e)
				...
				for (t = j.slice(0, r), c = a = 0, g = p.length; a < g; a++)
					if (t.indexOf(o = p.charAt(a)) < 0) {
						...
						} else if (!f && (p == p.toUpperCase() && (p = p.toLowerCase()) || p == p.toLowerCase() && (
								p = p.toUpperCase()))) {
							f = !0, a = -1, c = 0;
							continue
						}
		...
		function H(e, r, n, t) {
			...
			if (i = e.c[0], s = e.e, null == r) u = p(e.c), u = 1 == t || 2 == t && (s <= I || s >= T) ? v(u, s) :
				N(u, s, "0");
			else if (o = (e = X(new z(e), r, n)).e, f = (u = p(e.c)).length, 1 == t || 2 == t && (r <= o || o <=
					I)) {
				for (; f < r; u += "0", f++);
				u = v(u, o)
		...
		function V(e, r) {
			for (var n, t, i = 1, o = new z(e[0]); i < e.length; i++)(!(t = new z(e[i])).s || (n = w(o, t)) === r ||
				0 === n && o.s === r) && (o = t);
			return o
		}
		...
		function X(e, r, n, o) {
			...
			if (d) {
				e: {
					...
					if (o = o || r < 0 || null != d[p + 1] || (a < 0 ? g : g % m[s - a - 1]), o = n < 4 ? (w ||
							o) && (0 == n || n == (e.s < 0 ? 3 : 2)) : w > 5 || 5 == w && (4 == n || o || 6 == n &&
							(l >
								0 ? a > 0 ? g / m[s - a] : 0 : d[p - 1]) % 10 & 1 || n == (e.s < 0 ? 8 : 7)), r <
						1 || !
						d[0]) return d.length = 0,
					o ? (r -= e.e + 1, d[0] = m[(u - r % u) % u], e.e = -r || 0) : d[0] = e.e = 0,
					e;
					if (0 == l ? (d.length = p, h = 1, p--) : (d.length = p + 1, h = m[u - l], d[p] = a > 0 ? i(g /
							m[s - a] % m[a]) * h : 0), o)
						for (;;) {
							if (0 == p) {
		...
		function Y(e) {
			var r, n = e.e;
			return null === n ? e.toString() : (r = p(e.c), r = n <= I || n >= T ? v(r, n) : N(r, n, "0"), e.s < 0 ?
				"-" + r : r)
		}
		return z.clone = e, z.ROUND_UP = 0, z.ROUND_DOWN = 1, z.ROUND_CEIL = 2, z.ROUND_FLOOR = 3, z.ROUND_HALF_UP =
			4, z.ROUND_HALF_DOWN = 5, z.ROUND_HALF_EVEN = 6, z.ROUND_HALF_CEIL = 7, z.ROUND_HALF_FLOOR = 8, z
			.EUCLID = 9, z.config = z.set = function(e) {
				var r, n;
				if (null != e) {
					if ("object" != typeof e) throw Error(o + "Object expected: " + e);
					if (e.hasOwnProperty(r = "DECIMAL_PLACES") && (d(n = e[r], 0, h, r), L = n), e.hasOwnProperty(
							r = "ROUNDING_MODE") && (d(n = e[r], 0, 8, r), U = n), e.hasOwnProperty(r =
							"EXPONENTIAL_AT") && ((n = e[r]) && n.pop ? (d(n[0], -h, 0, r), d(n[1], 0, h, r), I = n[
							0], T = n[1]) : (d(n, -h, h, r), I = -(T = n < 0 ? -n : n))), e.hasOwnProperty(r =
							"RANGE"))
						...
						if (n) {
							if ("undefined" == typeof crypto || !crypto || !crypto.getRandomValues && !crypto
								.randomBytes) throw G = !n, Error(o + "crypto unavailable");
							G = n
						} else G = n
					}
					if (e.hasOwnProperty(r = "MODULO_MODE") && (d(n = e[r], 0, 9, r), F = n), e.hasOwnProperty(r =
							"POW_PRECISION") && (d(n = e[r], 0, h, r), k = n), e.hasOwnProperty(r = "FORMAT")) {
						if ("object" != typeof(n = e[r])) throw Error(o + r + " not an object: " + n);
						q = n
					}
					if (e.hasOwnProperty(r = "ALPHABET")) {
						if ("string" != typeof(n = e[r]) || /^.?$|[+\-.\s]|(.).*\1/.test(n)) throw Error(o + r +
							" invalid: " + n);
				...
				if (null == e ? e = L : d(e, 0, h), f = t(e / u), G)
					if (crypto.getRandomValues) {
						for (r = crypto.getRandomValues(new Uint32Array(f *= 2)); a < f;)(l = 131072 * r[a] + (
							r[a + 1] >>> 11)) >= 9e15 ? (n = crypto.getRandomValues(new Uint32Array(2)), r[
							a] = n[0], r[a + 1] = n[1]) : (g.push(l % 1e14), a += 2);
						a = f / 2
					} else {
						if (!crypto.randomBytes) throw G = !1, Error(o + "crypto unavailable");
						for (r = crypto.randomBytes(f *= 7); a < f;)(l = 281474976710656 * (31 & r[a]) +
								1099511627776 * r[a + 1] + 4294967296 * r[a + 2] + 16777216 * r[a + 3] + (r[a +
									4] << 16) + (r[a + 5] << 8) + r[a + 6]) >= 9e15 ? crypto.randomBytes(7)
							.copy(r, a) : (g.push(l % 1e14), a += 7);
						a = f / 7
					} if (!G)
					for (; a < f;)(l = A()) < 9e15 && (g[a++] = l % 1e14);
				for (f = g[--a], e %= u, f && e && (l = c[u - e], g[a] = i(f / l) * l); 0 === g[a]; g.pop(),
					a--);
				if (a < 0) g = [s = 0];
				else {
				...
				function r(e, r, n, t) {
					for (var i, o, s = [0], f = 0, u = e.length; f < u;) {
						...
						for (s[0] += t.indexOf(e.charAt(f++)), i = 0; i < s.length; i++) s[i] > n - 1 && (null == s[
							i + 1] && (s[i + 1] = 0), s[i + 1] += s[i] / n | 0, s[i] %= n)
					}
					return s.reverse()
				}
				return function(n, t, i, o, s) {
					var f, u, l, c, a, h, g, w, d = n.indexOf("."),
						m = L,
						v = U;
					for (d >= 0 && (c = k, k = 0, n = n.replace(".", ""), h = (w = new z(t)).pow(n.length - d),
							k = c, w.c = r(N(p(h.c), h.e, "0"), 10, i, e), w.e = w.c.length), l = c = (g = r(n,
							t, i, s ? (f = j, e) : (f = e, j))).length; 0 == g[--c]; g.pop());
					if (!g[0]) return f.charAt(0);
					if (d < 0 ? --l : (h.c = g, h.e = l, h.s = o, g = (h = O(h, w, m, v, i)).c, a = h.r, l = h
							.e), d = g[u = l + m + 1], c = i / 2, a = a || u < 0 || null != g[u + 1], a = v <
						4 ? (null != d || a) && (0 == v || v == (h.s < 0 ? 3 : 2)) : d > c || d == c && (4 ==
							v || a || 6 == v && 1 & g[u - 1] || v == (h.s < 0 ? 8 : 7)), u < 1 || !g[0]) n = a ?
						N(f.charAt(1), -m, f.charAt(0)) : f.charAt(0);
					else {
						if (g.length = u, a)
				...
				function e(e, r, n) {
					var t, i, o, s, f = 0,
						...
						l = r % a,
						c = r / a | 0;
					for (e = e.slice(); u--;) f = ((i = l * (o = e[u] % a) + (t = c * o + (s = e[u] / a | 0) * l) %
						a * a + f) / n | 0) + (t / a | 0) + c * s, e[u] = i % n;
				...
				return function(t, o, s, l, c) {
					var a, h, p, w, d, m, v, N, O, y, b, E, A, S, R, _, B, D = t.s == o.s ? 1 : -1,
						P = t.c,
						x = o.c;
					if (!(P && P[0] && x && x[0])) return new z(t.s && o.s && (P ? !x || P[0] != x[0] : x) ?
						P && 0 == P[0] || !x ? 0 * D : D / 0 : NaN);
					for (O = (N = new z(D)).c = [], D = s + (h = t.e - o.e) + 1, c || (c = f, h = g(t.e / u) -
							g(o.e / u), D = D / u | 0), p = 0; x[p] == (P[p] || 0); p++);
					if (x[p] > (P[p] || 0) && h--, D < 0) O.push(1), w = !0;
					else {
						for (S = P.length, _ = x.length, p = 0, D += 2, (d = i(c / (x[0] + 1))) > 1 && (x = e(x,
								d, c), P = e(P, d, c), _ = x.length, S = P.length), A = _, b = (y = P.slice(0,
								_)).length; b < _; y[b++] = 0);
						B = x.slice(), B = [0].concat(B), R = x[0], x[1] >= c / 2 && R++;
						do {
							if (d = 0, (a = r(x, y, _, b)) < 0) {
								if (E = y[0], _ != b && (E = E * c + (y[1] || 0)), (d = i(E / R)) > 1)
									for (d >= c && (d = c - 1), v = (m = e(x, d, c)).length, b = y.length; 1 ==
										r(m, y, v, b);) d--, n(m, _ < v ? B : x, v, c), v = m.length, a = 1;
			...
			}(), S = /^(-?)0([xbo])(?=\w[\w.]*$)/i, R = /^([^.]+)\.$/, _ = /^\.([^.]+)$/, B = /^-?(Infinity|NaN)$/,
			D = /^\s*\+(?=[\w.])|^\s+|\s+$/g, b = function(e, r, n, t) {
				...
				if (null != r && (r = new z(r)), c = e.e > 14, !p.c || !p.c[0] || 1 == p.c[0] && !p.e && 1 == p.c
					.length || !e.c || !e.c[0]) return g = new z(Math.pow(+Y(p), c ? e.s * (2 - m(e)) : +Y(e))), r ?
					g.mod(r) : g;
				if (a = e.s < 0, r) {
					if (r.c ? !r.c[0] : !r.s) return new z(NaN);
					(s = !a && p.isInteger() && r.isInteger()) && (p = p.mod(r))
				} else {
					if (e.e > 9 && (p.e > 0 || p.e < -1 || (0 == p.e ? p.c[0] > 1 || c && p.c[1] >= 24e7 : p.c[0] <
							8e13 || c && p.c[0] <= 9999975e7))) return l = p.s < 0 && m(e) ? -0 : 0, p.e > -1 && (
						l = 1 / l), new z(a ? 1 / l : l);
					k && (l = t(k / u + 2))
				}
				for (c ? (n = new z(.5), a && (e.s = 1), h = m(e)) : h = (f = Math.abs(+Y(e))) % 2, g = new z(
						x);;) {
					if (h) {
				...
				return s ? g : (a && (g = x.div(g)), r ? g.mod(r) : l ? X(g, k, U, undefined) : g)
				...
				var n, t, i = this;
				return e = new z(e, r), !i.c || !e.s || e.c && !e.c[0] ? new z(NaN) : !e.c || i.c && !i.c[0] ?
					new z(i) : (9 == F ? (t = e.s, e.s = 1, n = O(i, e, 0, 3), e.s = t, n.s *= t) : n = O(i, e, 0,
						F), (e = i.minus(n.times(e))).c[0] || 1 != F || (e.s = i.s), e)
			}, P.multipliedBy = P.times = function(e, r) {
				var n, t, i, o, s, l, c, h, p, w, d, m, v, N, O, y = this,
					b = y.c,
					E = (e = new z(e, r)).c;
				if (!(b && E && b[0] && E[0])) return !y.s || !e.s || b && !b[0] && !E || E && !E[0] && !b ? e.c = e
					.e = e.s = null : (e.s *= y.s, b && E ? (e.c = [0], e.e = 0) : e.c = e.e = null), e;
				for (t = g(y.e / u) + g(e.e / u), e.s *= y.s, (c = b.length) < (w = E.length) && (v = b, b = E, E =
						v, i = c, c = w, w = i), i = c + w, v = []; i--; v.push(0));
				for (N = f, O = a, i = w; --i >= 0;) {
					for (n = 0, d = E[i] % O, m = E[i] / O | 0, o = i + (s = c); o > i;) n = ((h = d * (h = b[--s] %
							O) + (l = m * h + (p = b[s] / O | 0) * d) % O * O + v[o] + n) / N | 0) + (l / O | 0) +
						m * p, v[o--] = h % N;
					v[o] = n
				}
				...
				for ((i = l.length) - (r = c.length) < 0 && (n = c, c = l, l = n, r = i), i = 0; r;) i = (l[--r] =
					l[r] + c[r] + i) / f | 0, l[r] = f === l[r] ? 0 : l[r] % f;
				...
				if (0 == (f = Math.sqrt(+Y(o))) || f == 1 / 0 ? (((r = p(s)).length + u) % 2 == 0 && (r += "0"), f =
						Math.sqrt(+r), u = g((u + 1) / 2) - (u < 0 || u % 2), n = new z(r = f == 1 / 0 ? "5e" + u :
							(r = f.toExponential()).slice(0, r.indexOf("e") + 1) + u)) : n = new z(f + ""), n.c[0])
					for ((f = (u = n.e) + l) < 3 && (f = 0);;)
						if (i = n, n = c.times(i.plus(O(o, i, l, 1))), p(i.c).slice(0, f) === (r = p(n.c)).slice(0,
								f)) {
				...
				if (t = i.toFixed(e, r), i.c) {
					...
					t = h ? a + (n.decimalSeparator || "") + ((l = +n.fractionGroupSize) ? h.replace(new RegExp(
						"\\d{" + l + "}\\B", "g"), "$&" + (n.fractionGroupSeparator || "")) : h) : a
				}
				return (n.prefix || "") + t + (n.suffix || "")
			}, P.toFraction = function(e) {
				var r, n, t, i, s, f, l, a, h, g, w, d, m = this,
					v = m.c;
				if (null != e && (!(l = new z(e)).isInteger() && (l.c || 1 !== l.s) || l.lt(x))) throw Error(o +
					"Argument " + (l.isInteger() ? "out of range: " : "not an integer: ") + Y(l));
				if (!v) return new z(m);
				for (r = new z(x), h = n = new z(x), t = a = new z(x), d = p(v), s = r.e = d.length - m.e - 1, r.c[
						0] = c[(f = s % u) < 0 ? u + f : f], e = !e || l.comparedTo(r) > 0 ? s > 0 ? r : h : l, f =
					M, M = 1 / 0, l = new z(d), a.c[0] = 0; g = O(l, r, 0, 1), 1 != (i = n.plus(g.times(t)))
					.comparedTo(e);) n = t, t = i, h = a.plus(g.times(i = h)), a = i, r = l.minus(g.times(i = r)),
					l = i;
				return i = O(e.minus(n), t, 0, 1), a = a.plus(i.times(h)), n = n.plus(i.times(t)), a.s = h.s = m.s,
					w = O(h, t, s *= 2, U).minus(m).abs().comparedTo(O(a, n, s, U).minus(m).abs()) < 1 ? [h, t] : [
						a, n
					], M = f, w
			}, P.toNumber = function() {
				return +Y(this)
				...
				var r, n = this,
					t = n.s,
					i = n.e;
				return null === i ? t ? (r = "Infinity", t < 0 && (r = "-" + r)) : r = "NaN" : (null == e ? r = i <=
					I || i >= T ? v(p(n.c), i) : N(p(n.c), i, "0") : 10 === e && $ ? r = N(p((n = X(new z(n),
						L + i + 1, U)).c), n.e, "0") : (d(e, 2, j.length, "Base"), r = y(N(p(n.c), i, "0"), 10,
						e, t, !0)), t < 0 && n.c[0] && (r = "-" + r)), r
